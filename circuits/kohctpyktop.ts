...
const kohctpyktop: CircuitSimulationFactories = {
      ...
      items: [
        {
          ...
          label: '01 - KT411I - QUAD INVERTER GATE',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '02 KT221A DUAL 2-INPUT AND GATE',
          label: '02 - KT221A - DUAL 2-INPUT AND GATE',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '03 KT141AO 4-INPUT AND-OR GATE',
          label: '03 - KT141AO - 4-INPUT AND-OR GATE',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '04 KO229 POWER ON RESET GENERATOR',
          label: '04 - KO229 - POWER ON RESET GENERATOR',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '06 KL2S1 DUAL SET-RESET LATCH',
          label: '06 - KL2S1 - DUAL SET-RESET LATCH',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
      ...
      items: [
        {
          ...
          label: '07 - KL2T1 - DUAL TOGGLE LATCH',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '08 KO224X DUAL FREQUENCY OSCILLATOR',
          label: '08 - KO224X - DUAL FREQUENCY OSCILLATOR',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '09 KD124 2-TO-4 LINE DECODER',
          label: '09 - KD124 - 2-TO-4 LINE DECODER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '10 KA180 2-BIT ADDER WITH CARRY',
          label: '10 - KA180 - 2-BIT ADDER WITH CARRY',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '11 KC82F DIVIDE-BY-FOUR COUNTER',
          label: '11 - KC82F - DIVIDE-BY-FOUR COUNTER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
      ...
      items: [
        {
          ...
          label: '12 - KM141P - 4-TO-1 MULTIPLEXER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '13 KC84C 4-BIT COUNTER WITH CLEAR',
          label: '13 - KC84C - 4-BIT COUNTER WITH CLEAR',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '14 KC74S 4-BIT SHIFT REGISTER S-TO-P',
          label: '14 - KC74S - 4-BIT SHIFT REGISTER S-TO-P',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '15 KR8S1 8-BIT ADDRESSABLE SRAM',
          label: '15 - KR8S1 - 8-BIT ADDRESSABLE SRAM',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '16 KA181 2-BIT LOGICAL FUNCTION UNIT',
          label: '16 - KA181 - 2-BIT LOGICAL FUNCTION UNIT',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
      ...
      items: [
        {
          ...
          label: '17 - X901 - RADIO MESSAGE STREAM DECODER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
        ...
        {
          key: '18 X902 GRENADE LAUNCHER AMMO COUNTER',
          label: '18 - X902 - GRENADE LAUNCHER AMMO COUNTER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
            ...
            // LOW, Y0, Y1, Y2, Y3
            const [seqLOW, seqY0, seqY1, seqY2, seqY3] =
        ...
        {
          key: '19 X903 GATLING CANNON FIRE CONTROLLER',
          label: '19 - X903 - GATLING CANNON FIRE CONTROLLER',
          setup: (network) => {
            const pins = network.getPinNodes();
            if (pins.length !== 12) {
              throw new Error(`Pin count must be 12, got ${pins.length}`);
            ...
            // TRIG, A+, A-, B+, B-
            const [seqTRIG, seqA, seqAN, seqB, seqBN] =
              createSequencesFromInputs(
                [seqFIRE, seqLOCK],
