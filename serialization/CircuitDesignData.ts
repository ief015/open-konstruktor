...
export default class CircuitDesignData extends DesignData {
  ...
  protected rebuildPins(): void {
    ...
    for (let pin = 0; pin < numPinRows; pin++) {
      const row =
        pin * DEFAULT_PIN_SIZE + pin * DEFAULT_PIN_GAP + DEFAULT_PIN_OFFSET_Y;
      for (let x = 0; x < DEFAULT_PIN_SIZE; x++) {
        for (let y = 0; y < DEFAULT_PIN_SIZE; y++) {
          ...
          if (x < DEFAULT_PIN_SIZE - 1) {
            metalConnectionsHLayer[left + x][row + y] =
              ConnectionValue.Connected;
            metalConnectionsHLayer[right + x][row + y] =
              ConnectionValue.Connected;
          }
          if (y < DEFAULT_PIN_SIZE - 1) {
            metalConnectionsVLayer[left + x][row + y] =
              ConnectionValue.Connected;
            metalConnectionsVLayer[right + x][row + y] =
              ConnectionValue.Connected;
          }
        }
      }
  ...
  public getPinPoint(pin: number): [col: number, row: number] {
    ...
    const row =
      pinRow * DEFAULT_PIN_SIZE +
      pinRow * DEFAULT_PIN_GAP +
      DEFAULT_PIN_OFFSET_Y;
    const left = DEFAULT_PIN_OFFSET_X;
    const right = columns - DEFAULT_PIN_OFFSET_X - DEFAULT_PIN_SIZE;
  ...
  public override getDesignScore(): number {
  ...
  public isKOHCTPYKTOPCompatible(): boolean {
    return (
      this.layers.length == Layer.COUNT &&
  ...
  public static override from(data: Uint8Array): CircuitDesignData {
    ...
    for (let pin = 0, pinRows = 0; ; pin += 2) {
      ...
      design.layers[Layer.Metal][loc[0]][loc[1]] === MetalValue.Metal &&
        pinRows++;
      if (loc[1] >= rows - 1) {
