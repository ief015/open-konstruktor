...
export type DifferenceMethod = 'normal' | 'kohctpyktop';
...
export default class Sequence implements Iterable<boolean> {
   ...
   *
   * @param options.method The method to use for calculating the difference. Defaults to 'normal'.
   ...
   * frames that have a distance of 2 to state changes on the expected sequence. `other` should
   * be the sequence that is generated by the circuit. `this` should be the expected
   * sequence.
   *
   * @returns The number of frames that are different, and the ratio. If the ratio is 0, then
   * the sequences are identical. If the ratio is 1, then the sequences are completely different.
   */
  public getDifference(other: Sequence, options: DifferenceOptions = {}): DifferenceResult {
    ...
    return { differences: diff, ratio: diff / max };
  }
