...
export default class FieldGraph {
  ...
  public isValidGateSpot(point: Point, direction: Direction): boolean {
    ...
    if (direction === 'h') {
      ...
      if (prev !== center || center !== next) return false;
      if (
        data.get(Layer.SiliconConnectionsV, x, y) !== ConnectionValue.Connected
      )
        return false;
      if (
        data.get(Layer.SiliconConnectionsV, x, y - 1) !==
      ...
      if (center === data.get(Layer.Silicon, x + 1, y)) {
        if (
          data.get(Layer.SiliconConnectionsH, x, y) ===
          ConnectionValue.Connected
        ) {
          return false;
        }
      ...
      if (center === data.get(Layer.Silicon, x - 1, y)) {
        if (
          data.get(Layer.SiliconConnectionsH, x - 1, y) ===
          ConnectionValue.Connected
        ) {
          return false;
        }
      ...
      if (center === data.get(Layer.Silicon, x, y + 1)) {
        if (
          data.get(Layer.SiliconConnectionsV, x, y) ===
          ConnectionValue.Connected
        ) {
          return false;
        }
  ...
  public placeSilicon(type: SiliconType, startPoint: Point, lastPoint?: Point) {
    ...
    if (
      existing === SiliconValue.None &&
      x >= this.minDrawColumn &&
      x <= this.maxDrawColumn
    ) {
  ...
  public queryMetalConnections(point: Point): QueryMetalResult[] {
    ...
    for (const offset of offsets) {
      ...
      const layer =
        dir === 'h' ? Layer.MetalConnectionsH : Layer.MetalConnectionsV;
  ...
  public querySiliconConnections(point: Point): QuerySiliconResult[] {
    ...
    for (const offset of offsets) {
      const { cx, cy, dx, dy, dir } = offset;
      const layer =
        dir === 'h' ? Layer.SiliconConnectionsH : Layer.SiliconConnectionsV;
      if (data.get(layer, x + cx, y + cy) === ConnectionValue.Connected) {
        const ax = x + dx;
        const ay = y + dy;
  ...
  public queryGate(point: Point): Direction | false {
    const { data } = this;
    const [x, y] = point;
    return data.get(Layer.GatesH, x, y) === GateValue.Gate
      ? 'h'
  ...
  public query(point: Point): QueryResult {
    ...
    const silicon =
      siliconRaw === SiliconValue.PSilicon
        ? 'p'
        : siliconRaw === SiliconValue.NSilicon
          ? 'n'
          : false;
  ...
  public clearRect(start: Point, end: Point, options: ClearOptions = {}) {
    ...
    const [x2, y2] = end;
    const minX = Math.max(
      enforceBounds ? this.minDrawColumn : 0,
      Math.min(x1, x2),
    );
    const minY = Math.max(0, Math.min(y1, y2));
    const maxX = Math.min(
      enforceBounds ? this.maxDrawColumn : columns - 1,
      Math.max(x1, x2),
    );
    ...
    for (const p of traceRectBorder(
      [minX - 1, minY - 1],
      [maxX + 1, maxY + 1],
    )) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let y = 0; y < height; y++) {
        ...
        copy.set(
          ...
          x,
          y,
          data.get(Layer.Metal, ax, ay) ?? MetalValue.None,
        );
        copy.set(
          Layer.Silicon,
          x,
          y,
          data.get(Layer.Silicon, ax, ay) ?? SiliconValue.None,
        );
        copy.set(
          Layer.Vias,
          x,
          y,
          data.get(Layer.Vias, ax, ay) ?? ViaValue.None,
        );
        copy.set(
          Layer.GatesH,
          x,
          y,
          data.get(Layer.GatesH, ax, ay) ?? GateValue.None,
        );
        copy.set(
          Layer.GatesV,
          x,
          y,
          data.get(Layer.GatesV, ax, ay) ?? GateValue.None,
        );
        if (x < width - 1) {
          copy.set(
            Layer.MetalConnectionsH,
            x,
            y,
  ...
  public canPaste(leftTop: Point, graph: FieldGraph): boolean {
    ...
    const { columns: otherColumns, rows: otherRows } =
      otherData.getDimensions();
    ...
    for (let x = 0; x < otherColumns; x++) {
      ...
      for (let y = 0; y < otherRows; y++) {
        ...
        if (ax >= this.minDrawColumn && ax <= this.maxDrawColumn) {
          if (
            overwrite ||
            otherData.get(Layer.Metal, x, y) !== MetalValue.None
          ) {
            myData.set(Layer.Metal, ax, ay, otherData.get(Layer.Metal, x, y));
          }
          if (
            overwrite ||
            otherData.get(Layer.Silicon, x, y) !== SiliconValue.None
          ) {
            myData.set(
              Layer.Silicon,
              ax,
              ay,
              otherData.get(Layer.Silicon, x, y),
            );
          }
          ...
          if (
            overwrite ||
            otherData.get(Layer.GatesH, x, y) !== GateValue.None
          ) {
            myData.set(Layer.GatesH, ax, ay, otherData.get(Layer.GatesH, x, y));
          }
          if (
            overwrite ||
            otherData.get(Layer.GatesV, x, y) !== GateValue.None
          ) {
            myData.set(Layer.GatesV, ax, ay, otherData.get(Layer.GatesV, x, y));
          }
          if (
            overwrite ||
            otherData.get(Layer.MetalConnectionsH, x, y) !==
              ConnectionValue.None
          ) {
            myData.set(
              Layer.MetalConnectionsH,
              ax,
          ...
          if (
            overwrite ||
            otherData.get(Layer.MetalConnectionsV, x, y) !==
              ConnectionValue.None
          ) {
            myData.set(
              Layer.MetalConnectionsV,
          ...
          if (
            overwrite ||
            otherData.get(Layer.SiliconConnectionsH, x, y) !==
              ConnectionValue.None
          ) {
            myData.set(
              Layer.SiliconConnectionsH,
          ...
          if (
            overwrite ||
            otherData.get(Layer.SiliconConnectionsV, x, y) !==
              ConnectionValue.None
          ) {
            myData.set(
              Layer.SiliconConnectionsV,
    ...
    for (let x = 0; x < columns; x++) {
      ...
      for (let y = 0; y < rows; y++) {
        ...
        dest.set(
          ...
          ax - 1,
          ay,
          this.data.get(Layer.MetalConnectionsH, x, y),
        );
        dest.set(
          Layer.MetalConnectionsV,
          ax,
          ay,
          this.data.get(Layer.MetalConnectionsV, x, y),
        );
        dest.set(
          Layer.SiliconConnectionsH,
          ax - 1,
          ay,
          this.data.get(Layer.SiliconConnectionsH, x, y),
        );
        dest.set(
          Layer.SiliconConnectionsV,
          ax,
          ay,
          this.data.get(Layer.SiliconConnectionsV, x, y),
        );
      }
    }
    this.data = dest;
