...
describe('01 KT411I QUAD INVERTER GATE', () => {
  test('empty', () => {
    ...
    const sim = getLoader('01 KT411I QUAD INVERTER GATE').setup(net);
    sim.run(280);
  ...
  test('passing', () => {
    const { getLoader } = useCircuitLoaders();
    const saveString =
      '';
...
describe('02 KT221A DUAL 2-INPUT AND GATE', () => {
  ...
  test('passing-2', () => {
    const { getLoader } = useCircuitLoaders();
    const saveString =
      '';
    const field = FieldGraph.from(saveString, 'circuit');
    const net = Network.from(field);
    const sim = getLoader('02 KT221A DUAL 2-INPUT AND GATE').setup(net);
    sim.run(280);
...
describe('03 KT141AO 4-INPUT AND-OR GATE', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('03 KT141AO 4-INPUT AND-OR GATE').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('04 KO229 POWER ON RESET GENERATOR', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('04 KO229 POWER ON RESET GENERATOR').setup(net);
    sim.run(80);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('05 KO223 DUAL FIXED FREQUENCY OSCILLATOR', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('05 KO223 DUAL FIXED FREQUENCY OSCILLATOR').setup(
      net,
    );
    sim.run(280);
    const verification = sim.verify();
...
describe('06 KL2S1 DUAL SET-RESET LATCH', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('06 KL2S1 DUAL SET-RESET LATCH').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('07 KL2T1 DUAL TOGGLE LATCH', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('07 KL2T1 DUAL TOGGLE LATCH').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('08 KO224X DUAL FREQUENCY OSCILLATOR', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('08 KO224X DUAL FREQUENCY OSCILLATOR').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('09 KD124 2-TO-4 LINE DECODER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('09 KD124 2-TO-4 LINE DECODER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('10 KA180 2-BIT ADDER WITH CARRY', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('10 KA180 2-BIT ADDER WITH CARRY').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('11 KC82F DIVIDE-BY-FOUR COUNTER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('11 KC82F DIVIDE-BY-FOUR COUNTER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('12 KM141P 4-TO-1 MULTIPLEXER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('12 KM141P 4-TO-1 MULTIPLEXER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
  ...
  test('passing', () => {
    const { getLoader } = useCircuitLoaders();
    const saveString =
      '';
    const field = FieldGraph.from(saveString, 'circuit');
    const net = Network.from(field);
    const sim = getLoader('12 KM141P 4-TO-1 MULTIPLEXER').setup(net);
    sim.run(280);
...
describe('13 KC84C 4-BIT COUNTER WITH CLEAR', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('13 KC84C 4-BIT COUNTER WITH CLEAR').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('14 KC74S 4-BIT SHIFT REGISTER S-TO-P', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('14 KC74S 4-BIT SHIFT REGISTER S-TO-P').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('15 KR8S1 8-BIT ADDRESSABLE SRAM', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('15 KR8S1 8-BIT ADDRESSABLE SRAM').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('16 KA181 2-BIT LOGICAL FUNCTION UNIT', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('16 KA181 2-BIT LOGICAL FUNCTION UNIT').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('17 X901 RADIO MESSAGE STREAM DECODER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('17 X901 RADIO MESSAGE STREAM DECODER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('18 X902 GRENADE LAUNCHER AMMO COUNTER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('18 X902 GRENADE LAUNCHER AMMO COUNTER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
...
describe('19 X903 GATLING CANNON FIRE CONTROLLER', () => {
  test('empty', () => {
    ...
    const field = new FieldGraph();
    const net = Network.from(field);
    const sim = getLoader('19 X903 GATLING CANNON FIRE CONTROLLER').setup(net);
    sim.run(280);
    const verification = sim.verify();
    const grade = verification.gradePercent;
